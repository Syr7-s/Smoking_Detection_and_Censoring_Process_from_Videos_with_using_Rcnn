...
% Last Modified by GUIDE v2.5 12-Sep-2020 12:54:06
...
uiwait(msgbox("RCNN ile Video üzerinde sigara nesnesini engelleme","Sigara Nesne Tespiti","custom",iconum));
...
% --- Executes on button press in yukleBtn.
function yukleBtn_Callback(hObject, eventdata, handles)
global videoObj
try
    ...
    f1=msgbox(MException.message+"Uygulamayý Sonlandýrýnýz"," Hata Olustu...","error");
end
...
% --- Executes on button press in baslatBtn.
function baslatBtn_Callback(hObject, eventdata, handles)
...
% --- Executes on button press in durdurBtn.
function durdurBtn_Callback(hObject, eventdata, handles)
...
% --- Executes on button press in kapatBtn.
function kapatBtn_Callback(hObject, eventdata, handles)
closereq();
...
% --- Executes on button press in agSecBtn.
function agSecBtn_Callback(hObject, eventdata, handles)
durum1=handles.radiobutton1.Value;  %RCNN 15 KATMAN Benim eðittiðim Að
durum2=handles.radiobutton2.Value; % RCNN 13 KATMAN Ýsanýn Eðittiði Að
try
...
if(durum2)
    ...
    f1=msgbox(MException.message+"Uygulamayý Tekrar Çalýþtýrýnýz"," Hata Olustu...","error");
end
